...
static int set_car_num_part_a(u32 parm)
{
    ...
    {
        video_rec_osd_buf[0] = ' ';
        video_rec_osd_buf[1] = ((u8)(parm >> 16))&0xff;
    }
...
static int set_car_num_part_b(u32 parm)
{
    ...
    //if (db_select("dat")) {
    //    video_rec_osd_buf[9] = '\\';
    //    memcpy(video_rec_osd_buf + 10, osd_str_buf, strlen(osd_str_buf));
    //}
    //printf("set_car_num: %s\n", video_rec_osd_buf);
    return 0;
}
static int set_car_num_part_c(u32 parm)
{
    //video_rec_osd_buf[4] = (u8)(parm >> 24);
    //video_rec_osd_buf[5] = (u8)(parm >> 16);
    //video_rec_osd_buf[6] = (u8)(parm >> 8);
    video_rec_osd_buf[8] = (u8)(parm >> 0);
...
static int num_set_function(u32 parm)
{
    if (!parm) {
	    ...
	    set_car_num_part_c(db_select("cnc"));	
    }
