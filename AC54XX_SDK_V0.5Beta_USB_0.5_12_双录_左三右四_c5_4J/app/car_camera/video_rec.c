...
#define SMALL_SCREEN_W          128 //16 aline
#define SMALL_SCREEN_H          96 //16 aline
#define SMALL_SCREEN_X          192
#define SMALL_SCREEN_Y          0


#define MOTION_STOP_SEC         15
#define MOTION_START_SEC        1
...
static int ve_mdet_start()
{
    ...
    **/
    ...
    ve_req.md_params.move_delay_ms = MOTION_START_SEC * 10;
...
static void video_rec_rename_file(int id, FILE *file, int fsize, int format)
{
    ...
    if (present >= 90 && present <= 110) {
        if (format == VIDEO_FMT_MOV) {
            sprintf(file_name, "DCIM/%d/vid_***.mov", (id == 0) ? 1 : 2);
        } else if (format == VIDEO_FMT_AVI) {
            sprintf(file_name, "DCIM/%d/vid_***.avi", (id == 0) ? 1 : 2);
        }
...
static int video_rec_del_old_file(int id)
{
	...
	if((cyc_time == 0) && ( cur_space  <= 1*(need_space / 1024) ) )   //by peng ?????????????￡???????????
	{
		printf("=======================================\n");
		return 0xff;
	}
	...
	if(cyc_time != 0)
	{
		...
		file = video_rec_get_first_file(id);
	    if (file) {
...
static int video0_rec_start()
{
    ...
    */
    req.rec.roi.roio_xy = (req.rec.height * 6 / 6 / 16) << 24 | (req.rec.height * 6 / 6 / 16) << 16 | (req.rec.width * 6 / 6 / 16) << 8 | (req.rec.width) * 6 / 6 / 16;
    req.rec.roi.roi1_xy = (req.rec.height * 12 / 12 / 16) << 24 | (req.rec.height * 12 / 12 / 16) << 16 | (req.rec.width * 12 / 12 / 16) << 8 | (req.rec.width) * 12 / 12 / 16;
...
struct imc_presca_ctl *video_disp_get_pctl()
{
    ...
    imc_presca_t.presca_en = true;
    imc_presca_t.gs_parma = 30;
    imc_presca_t.gs_parmb = 30;
    imc_presca_t.gs_parmc = 30;
    imc_presca_t.gs_parmd = 30;
...
static int video_rec_savefile(int dev_id)
{
    ...
    if (err) {
		__this->state = VIDREC_STA_STOP;   //  清除状态
		if(err == 0xff)   // by peng ???????
		{
			__this->state = VIDREC_STA_STOP;
			video_rec_post_msg("fsfull");	
...
static int video_disp_control()
{
    ...
    case DISP_BACK_WIN:
        ...
        if (__this->video2_online) {
            err = video0_disp_start(SMALL_SCREEN_W, SMALL_SCREEN_H, 0, 0);
        } else {
            err = video0_disp_start(SCREEN_W, SCREEN_H + SCREEN_H_OFFSET, 0, 0);
        }
#endif
...
#ifdef CONFIG_UI_STYLE_LY_LONGSCREEN_ENABLE
        err = video1_disp_start((SCREEN_W - SMALL_SCREEN_W), SCREEN_H, SMALL_SCREEN_W, 0);
#else
...
#ifdef CONFIG_UI_STYLE_LY_LONGSCREEN_ENABLE
        err = video2_disp_start((SCREEN_W - SMALL_SCREEN_W), SCREEN_H, SMALL_SCREEN_W, 0);
#else
...
static int video_rec_park_wait(void *priv)
{
    ...
    if (__this->state == VIDREC_STA_START) {
        #ifdef GSENSOR_UP   //有acc的，1分钟再关机，樊开要求
        	sys_timeout_add(NULL, video_rec_park_call_back, 60 * 1000);
		#else
...
static int video_rec_init()
{
    ...
    err = video0_disp_start(SCREEN_W, SCREEN_H + SCREEN_H_OFFSET, 0, 0);
#else
#ifndef CONFIG_UI_STYLE_LY_LONGSCREEN_ENABLE
...
static int video_park()
{

	if (db_select("par")) 
	{
		//puts("par on\n");
		video_rec_post_msg("offPARK");
	} else {
		//puts("par off\n");
		video_rec_post_msg("onPARK");
	}
...
static int video_rec_device_event_handler(struct sys_event *event)
{
    ...
    if (!ASCII_StrCmp(event->arg, "sd*", 4)) {
...
#else
                video0_disp_stop();
                video0_disp_start(SMALL_SCREEN_W, SMALL_SCREEN_H, 0, 0);
                video1_disp_start((SCREEN_W - SMALL_SCREEN_W), SCREEN_H, SMALL_SCREEN_W, 0);
#endif
                __this->disp_state = DISP_BIG_SMALL;
                if (__this->state == VIDREC_STA_START) {
                    video_rec_control(0);
        ...
        case DEVICE_EVENT_OUT:
            if (__this->video1_online) {
                ...
                if (__this->disp_state == DISP_BIG_SMALL) {
                    ...
                    video0_disp_stop();
                    video1_disp_stop();
                    video0_disp_start(SCREEN_W, SCREEN_H, 0, 0);
#endif
                    __this->disp_state = DISP_FRONT_WIN;
                } else if (__this->disp_state == DISP_HALF_WIN ||
                           __this->disp_state == DISP_BACK_WIN) {
        ...
        case DEVICE_EVENT_ONLINE:
            if (!__this->video2_online) {
                ...
                video0_disp_stop();
                video0_disp_start(SMALL_SCREEN_W, SMALL_SCREEN_H, 0, 0);
                video2_disp_start((SCREEN_W - SMALL_SCREEN_W), SCREEN_H, SMALL_SCREEN_W, 0);
#endif
                __this->disp_state = DISP_BIG_SMALL;
                if (__this->state == VIDREC_STA_START) {
                    video_rec_control(0);
	...
	else if (!strcmp(event->arg, "gsen_err"))
	{
		video_rec_post_msg("gsenerr");	
	}
