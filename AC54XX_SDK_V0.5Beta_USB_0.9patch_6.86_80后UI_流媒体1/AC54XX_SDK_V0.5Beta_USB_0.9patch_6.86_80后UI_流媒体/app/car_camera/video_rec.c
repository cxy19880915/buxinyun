...
#define VREC0_FBUF_SIZE     (7*1024*1024) //6
#define VREC1_FBUF_SIZE     (3*1024*1024) //2
#define VREC2_FBUF_SIZE     (3*1024*1024) //2
...
static void ve_server_event_handler(void *priv, int argc, int *argv)
{
    ...
    case VE_MSG_MOTION_DETECT_MOVING:
        ...
        if ((__this->state == VIDREC_STA_STOP) || (__this->state == VIDREC_STA_IDLE)) {
            ...
            move_start = 1;
            pmsg("VE_MSG_MOTION_DETECT_MOVING\n");
            video_rec_control(0);
        }
...
static int video1_rec_osd_ctl(u8 onoff)
{
 ...
 int video1_disp_start(u16 width, u16 height, u16 xoff, u16 yoff)
{
