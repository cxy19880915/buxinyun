...
//static char icn85xx_ts.ictype = ICN85XX_WITH_FLASH_85;

unsigned short icnt87_sram_crc = 0;
unsigned short icnt87_sram_length = 0;
extern icn85xx_ts_data icn85xx_ts;
...
int icn85xx_goto_progmode(void)
{
    ...
    while(retry > 0)
    {
        ...
        if(ret == 0)
        {
    		//pmsg(" icn85xx_goto_progmode over\n");
    		return ret;

        }
        //pmsg(" iic test icn85xx_goto_progmode! retry = %d\n", retry);
        retry--;
        delay_2ms(2);
    }
...
int icn85xx_check_progmod(void)
{
    ...
    ret = icn85xx_prog_i2c_rxdata(0x040002, &ucTemp, 1);
     printf("icn85xx_check_progmod: 0x%x\n", ucTemp);
    if(ret < 0)
    {
...
void EarseFlash(unsigned char erase_index,u32 flash_addr)
{
    unsigned char ucTemp[4] = {0,0,0,0};
    FlashWriteEnable();
...
int  icn85xx_fw_update(void *fw)
{
    ...
    printf("\n 20141014 icn85xx_fw_download start\n");
...
//    delay_2ms(1);
   ...
   printf("\n icn85xx_fw_download ok \n");
    if((ICN85XX_WITH_FLASH_85 == icn85xx_ts.ictype) || (ICN85XX_WITH_FLASH_86 == icn85xx_ts.ictype))
    {
        icn85xx_erase_flash();
    //    return R_PROGRAM_ERR;
        ...
        if(icn85xx_crc_check(crc_fw, file_size) != 0)
        {
            printf("\n read flash data error, crc error\n");
            return R_PROGRAM_ERR;
        }
        else
        {
            printf("\n read flash data ok, crc ok\n");
        }
        while((FlashState(0)&0x01));
...
//        printf("\n ---1111---- \n");
       // if(icn85xx_bootfrom_flash() == 0)
        ...
        {
            printf("\n icn85xx_bootfrom_flash error 2 \n");
            icn85xx_update_status(R_STATE_ERR);
            return R_STATE_ERR;
        }
    }
    else if(ICN85XX_WITHOUT_FLASH == icn85xx_ts.ictype)
    {
...
int  icn87xx_fw_update(void *fw)
{
    ...
    if(ICN85XX_WITH_FLASH_87 == icn85xx_ts.ictype)
    {
    ...
    {
        if(ICN85XX_WITHOUT_FLASH_87 == icn85xx_ts.ictype)
        {
            printf("\n error!! file_size > sram_len\n");
            icn85xx_update_status(R_PROGRAM_ERR);
            icn85xx_close_fw();
            return R_PROGRAM_ERR;
        }
    ...
    if(ICN85XX_WITH_FLASH_87 == icn85xx_ts.ictype)
    {
        ...
        while(version != 0x87)
        {
            ...
            printf("after upgrdate Value: %x\n",version);
            delay_2ms(5);
        }
    }
    else if(ICN85XX_WITHOUT_FLASH_87 == icn85xx_ts.ictype)
    {
