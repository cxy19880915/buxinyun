...
#define USER_ISP_CFG_NIGHT_H    ((0 << 8) 	+ 100)//((0 << 8) 	- 0) //+ 100

#define USER_ISP_CFG_INDOOR_L   ((0 << 8) 	+ 130) //+ 130
#define USER_ISP_CFG_INDOOR_H   ((6 << 8) 	- 128) //(6 << 8)

#define USER_ISP_CFG_OUTCLUDY_L ((6 << 8) 	+ 128) //(6 << 8)
#else
#define USER_ISP_CFG_NIGHT_H    ((0 << 8) 	+ 450)//((0 << 8) 	- 0) //+ 100

#define USER_ISP_CFG_INDOOR_L   ((0 << 8) 	+ 500) //+ 130
#define USER_ISP_CFG_INDOOR_H   ((7 << 8) 	- 128) //(6 << 8)

#define USER_ISP_CFG_OUTCLUDY_L ((7 << 8) 	+ 128) //(6 << 8)
#endif
#define USER_ISP_CFG_OUTCLUDY_H ((13 << 8) 	- 128)

#define USER_ISP_CFG_OUTDOOR_L  ((13 << 8) 	+ 128)
...
#ifdef CONFIG_BOARD_MIPI_TOUCH_V1_DVP
unsigned char wdr_en[] = {0, 0, 1, 1};
#else
unsigned char wdr_en[] = {0, 0, 1, 0};
#endif

/* 0:该场景不开wdr(无论菜单是否打开wdr使能，该场景都不开wdr)
...
static void isp_scene_switch(void *arg, u8 force)
{
    ...
    extern int ispt_params_smooth_en(u8 en);  //����
    ispt_params_smooth_en(0);
