...
#if (LCD_TYPE == LCD_DSI_VDO_4LANE_1280x480_WT686_1)
//------------------------------------------------------//
// lcd command initial
//------------------------------------------------------//
//command list
...
*/
	...
	_W,DELAY(0),PACKET_DCS,SIZE(3),0xB6,0x15,0x15,

	_W,DELAY(0),PACKET_DCS,SIZE(21),0xB4,0x0A,0x08,0x12,0x10,0x0E,0x0C,0x00,0x00,0x00,0x03,0x00,0x03,0x03,0x03,0x03,0x03,0x03,0x03,0x04,0x06,

	_W,DELAY(0),PACKET_DCS,SIZE(21),0xB3,0x0B,0x09,0x13,0x11,0x0F,0x0D,0x00,0x00,0x00,0x03,0x00,0x03,0x03,0x03,0x03,0x03,0x03,0x03,0x05,0x07,

	_W,DELAY(0),PACKET_DCS,SIZE(13),0xB0,0x54,0x32,0x23,0x45,0x44,0x44,0x44,0x44,0x90,0x01,0x90,0x01,

	_W,DELAY(0),PACKET_DCS,SIZE(9),0xB1,0x32,0x84,0x02,0x83,0x30,0x01,0x6B,0x01,

	_W,DELAY(0),PACKET_DCS,SIZE(2),0xB2,0x73,

	_W,DELAY(0),PACKET_DCS,SIZE(11),0xBD,0x4E,0x0E,0x50,0x50,0x26,0x1D,0x00,0x14,0x42,0x03,

	_W,DELAY(0),PACKET_DCS,SIZE(18),0xB7,0x01,0x01,0x09,0x11,0x0D,0x55,0x19,0x19,0x21,0x1D,0x00,0x00,0x00,0x00,0x02,0xFF,0x3C,

	_W,DELAY(0),PACKET_DCS,SIZE(6),0xB8,0x23,0x01,0x30,0x34,0x13,

	_W,DELAY(0),PACKET_DCS,SIZE(5),0xB9,0xA0,0x22,0x00,0x44,

	_W,DELAY(0),PACKET_DCS,SIZE(3),0xBA,0x12,0x63,

	_W,DELAY(0),PACKET_DCS,SIZE(7),0xC1,0x0C,0x16,0x04,0x0C,0x10,0x04,

	_W,DELAY(0),PACKET_DCS,SIZE(3),0xC2,0x11,0x41,

	_W,DELAY(0),PACKET_DCS,SIZE(4),0xC3,0x22,0x31,0x04,

	_W,DELAY(0),PACKET_DCS,SIZE(6),0xC7,0x05,0x23,0x6B,0x49,0x00,

	_W,DELAY(0),PACKET_DCS,SIZE(2),0xC5,0x00,

	_W,DELAY(0),PACKET_DCS,SIZE(4),0xD0,0x37,0xFF,0xFF,

	_W,DELAY(0),PACKET_DCS,SIZE(5),0xD2,0x63,0x0B,0x08,0x88,

	_W,DELAY(0),PACKET_DCS,SIZE(12),0xD3,0x01,0x00,0x00,0x01,0x01,0x37,0x25,0x38,0x31,0x06,0x07,

	//_W,DELAY(0),PACKET_DCS,SIZE(39),0xC8,0x7C,0x6A,0x5D,0x53,0x53,0x45,0x4B,0x35,0x4D,0x4A,0x49,0x66,0x53,0x57,0x4A,0x48,0x3B,0x2A,0x06,0x7C,0x6A,0x5D,0x53,0x53,0x45,0x4B,0x35,0x4D,0x4A,0x49,0x66,0x53,0x57,0x4A,0x48,0x3B,0x2A,0x06,
	_W,DELAY(0),PACKET_DCS,SIZE(39),0xC8,0x7C,0x6e,0x64,0x5c,0x5e,0x51,0x5a,0x46,0x5f,0x5D,0x5D,0x7C,0x6B,0x75,0x6D,0x6B,0x5A,0x4F,0x06,0x7C,0x6e,0x64,0x5c,0x5e,0x51,0x5a,0x46,0x5f,0x5D,0x5D,0x7C,0x6B,0x75,0x6D,0x6B,0x5A,0x4F,0x06,
	_W,DELAY(0),PACKET_DCS,SIZE(9),0xC6,0x00,0x00,0xFF,0x00,0x00,0xFF,0x00,0x00,

	_W,DELAY(0),PACKET_DCS,SIZE(3),0xF4,0x08,0x77,
	#endif
...
REGISTER_LCD_DEVICE_DRIVE(dev)  = {
    .enable  = IF_ENABLE(LCD_DSI_VDO_4LANE_1280x480_WT686_1),
    .logo 	 = LCD_NAME,
    .type 	 = LCD_MIPI,
    .dev  	 = &mipi_dev_t,
