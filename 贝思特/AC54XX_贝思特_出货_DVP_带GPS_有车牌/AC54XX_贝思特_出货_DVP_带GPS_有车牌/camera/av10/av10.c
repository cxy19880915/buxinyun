...
static u8 avin_valid_signal(void)
{
    u8 status ;
...
void av10_init()
{
    ...
    av10_f_and(AV10_PORTA_DIR, ~BIT(3));
...
//        av10_f_or (AV10_PORTB_HD,0xff);  //test
    ...
    av10_f_and(AV10_PORTC_PD, ~(BIT(3) | BIT(4)));
    av10_f_and(AV10_PORTC_PU, ~(BIT(3) | BIT(4)));
    av10_f_and(AV10_PORTC_DIE, ~(BIT(3) | BIT(4)));
    av10_f_mov(AV10_CLK_CON0, BIT(0) | BIT(1) | (3 << 2) | BIT(4));//27M
#else
...
REGISTER_CAMERA(AV10) = {
    ...
    .mbus_config        =   SEN_MBUS_DATA_WIDTH_8B | SEN_MBUS_FIELD_EVEN_HIGH | SEN_MBUS_PCLK_SAMPLE_FALLING|SEN_MBUS_DATA_REVERSE,
    .fps         		= 	0,
