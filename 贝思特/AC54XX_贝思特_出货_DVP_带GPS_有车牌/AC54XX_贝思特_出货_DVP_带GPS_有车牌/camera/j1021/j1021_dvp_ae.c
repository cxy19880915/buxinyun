...
#include "j1021_dvp_ae.h"
#include "j1021_dvp.h"

#include "asm/isp_alg.h"


extern u32 j1021_dvp_calc_shutter(isp_ae_shutter_t *shutter, u32 exp_time_us, u32 gain);
extern u32 j1021_dvp_set_shutter(isp_ae_shutter_t *shutter);
extern void  *j1021_dvp_get_ae_curve(u32 type, u32 fps, int *min_ev, int *max_ev);


//Anti-flicker:50Hz
const static u32 j1021_dvp_ae_50hz_25fps[][AE_CURVE_INFO_MAX] = {
...
const static u32 j1021_dvp_ae_50hz_30fps[][AE_CURVE_INFO_MAX] = {
    // EV       ExpoTime    ISO     INTERPOLATE
    {9102000,     30,       100,    AE_INTERP_EXP},  // lv = 20
    {27306,    10000,       100,    AE_INTERP_GAIN}, // lv = 10.74
    {13653,    20000,       100,    AE_INTERP_GAIN},
    ...
    {13,       40000,     68264,    AE_INTERP_GAIN},
    {AE_CURVE_END, 0,         0,              0}
};

const static u32 j1021_dvp_ae_60hz_30fps[][AE_CURVE_INFO_MAX] = {
...
const static u32 j1021_dvp_ae_50hz_variable_fps[][AE_CURVE_INFO_MAX] = {
    // EV       ExpoTime    ISO     INTERPOLATE
/*
    {9102000,     30,       100,    AE_INTERP_EXP},  // lv = 20
    {27306,    10000,       100,    AE_INTERP_GAIN}, // lv = 10.74
...
static isp_ae_params_t  j1021_dvp_ae_params = {
    ...
    .ae_ops = {
        .calc_shutter = j1021_dvp_calc_shutter,
        .set_shutter = j1021_dvp_set_shutter,
        .get_ae_curve = j1021_dvp_get_ae_curve,
    },
    .ae_step = 1, // 1 //PPK ADD ON 20170726
};


void j1021_dvp_ae_ev_init(u32 fps)
{
    u32 time, gain;
    int minev, maxev;
    ...
    ae_curve = j1021_dvp_get_ae_curve(AE_CURVE_TYPE, fps, &minev, &maxev);
    isp_ae_curve_interp(ae_curve, AE_INIT_EV, &time, &gain);

    j1021_dvp_calc_shutter(&ae_shutter, time, gain);

    j1021_dvp_set_shutter(&ae_shutter);
}

void   *j1021_dvp_get_ae_curve(u32 type, u32 fps, int *min_ev, int *max_ev)
{
    void *curve;

    *min_ev = AE_MIN_EV;
    *max_ev = AE_MAX_EV;
    curve = &j1021_dvp_ae_50hz_25fps;
    ...
    if (type == AE_CURVE_50HZ) {
       ...
       // curve = &bg0806_mipi_ae_50hz_var_fps;
        curve = &j1021_dvp_ae_50hz_variable_fps;
        ...
        if (fps == 25) {
            ...
            curve = &j1021_dvp_ae_50hz_30fps;
        }
        #endif // J1021_VARIABLE_FPS
    } else {
        *min_ev = AE_MIN_EV;
        *max_ev = AE_MAX_EV;

        curve = &j1021_dvp_ae_60hz_30fps;
...
void *j1021_dvp_get_ae_params()
{
    return (void *)&j1021_dvp_ae_params;
}
