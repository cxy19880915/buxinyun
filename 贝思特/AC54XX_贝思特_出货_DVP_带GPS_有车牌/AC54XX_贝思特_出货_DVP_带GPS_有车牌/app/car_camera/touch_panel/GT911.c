...
//    }
    ...
    if ((strncmp((const char *)tpd_info.pid, "911", 3)) &&(strncmp((const char *)tpd_info.pid, "615", 3)))
	{
        printf("\nTch ID %s != \"911\"", tpd_info.pid);
        if(strncmp((const char *)tpd_info.pid, "915", 3) && strncmp((const char *)tpd_info.pid, "913", 3) && strncmp((const char *)tpd_info.pid, "968", 3))
        {
            return -1;     
        } 
    }
...
g_end:
    ...
    if (ret == TRUE) {
        if (_touch_panel_data->enable) {
            _touch_panel_timer_reg = sys_timer_add((void *)0, GT911_read_point, 5);
        }
    }
