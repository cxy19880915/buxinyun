 ...
 */
 ...
 static void power_off_play_end(void *_ui)
 {
	 ...
	 if (ui) {
 	  ...
 	  if((usb_is_charging())&&(ttt == 1))
    	{
    		printf("usb_is_charging()\n");
				
			//	if(get_power_off_state())  //如果是USB掉电关机的，此时已经重新上电，重新开机
			//	{
					cpu_reset();
			//	}
				WDT_CON &= ~BIT(4); //关闭看门狗
...
#endif
        if (!ASCII_StrCmp(event->arg, "accin", 5))  
        {
            pmsg("acc in\n");  
            it.name = "video_rec";
            it.action = ACTION_VIDEO_REC_ACCIN_CONTROL;
            start_app(&it);
        }
    ...
    case DEVICE_EVENT_OUT:
        ...
        if (!ASCII_StrCmp(event->arg, "accin", 5))  
        {
            ...
            if (app) 
            {
                if (!strcmp(app->name, "video_rec")) {
                    it.name = "video_rec";
                    it.action = ACTION_VIDEO_REC_ACCOUT_CONTROL;
                    start_app(&it);    
                    break;
                }
                if ((!strcmp(app->name, "video_photo"))
