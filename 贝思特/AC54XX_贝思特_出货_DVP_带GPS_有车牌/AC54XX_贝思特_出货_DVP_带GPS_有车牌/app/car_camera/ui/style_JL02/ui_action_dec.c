...
static int rep_file_res_handler(const char *type, u32 args)
{
    if (*type == 'w') {
        ...
        if (__this->file_type == 1) {
            if (args >= 1920) {
                /* 1080p */
                ui_text_set_str_by_id(DEC_TXT_RESOLUTION, "ascii", "1440P");
                __this->file_res = 0;
            } else if (args >= 1080) {
                /* 720p */
                ui_text_set_str_by_id(DEC_TXT_RESOLUTION, "ascii", "1296P");
                __this->file_res = 1;
            } else {
                /* vga */
                ui_text_set_str_by_id(DEC_TXT_RESOLUTION, "ascii", "1080P");
                __this->file_res = 2;
            }
        } else if (__this->file_type == 0) {
            if (args >= 4032) {
                /* 12M */
                ui_text_set_str_by_id(DEC_TXT_RESOLUTION, "ascii", "12M");
                __this->file_res = 10;
            } else if (args >= 3648) {
                /* 10M */
                ui_text_set_str_by_id(DEC_TXT_RESOLUTION, "ascii", "10M");
                __this->file_res = 9;
            } else if (args >= 3264) {
                /* 8M */
                ui_text_set_str_by_id(DEC_TXT_RESOLUTION, "ascii", "8M");
                __this->file_res = 8;
            } else if (args >= 2592) {
                /* 5M */
                ui_text_set_str_by_id(DEC_TXT_RESOLUTION, "ascii", "5M");
                __this->file_res = 7;
            } else if (args >= 2048) {
                /* 3M */
                ui_text_set_str_by_id(DEC_TXT_RESOLUTION, "ascii", "3M");
                __this->file_res = 6;
            } else if (args >= 1920) {
                /* 2M */
                ui_text_set_str_by_id(DEC_TXT_RESOLUTION, "ascii", "2M");
                __this->file_res = 5;
            } else if (args >= 1280) {
                /* 1.3M */
                ui_text_set_str_by_id(DEC_TXT_RESOLUTION, "ascii", "1M");
                __this->file_res = 4;
            } else {
                /* vga */
                ui_text_set_str_by_id(DEC_TXT_RESOLUTION, "ascii", "VGA");
                __this->file_res = 3;
            }
        }
    }
...
static void battery_event_handler(struct sys_event *event, void *priv)
{
    ...
    if (event->type == SYS_DEVICE_EVENT) {
        ...
        else if (!ASCII_StrCmp(event->arg, "disacc", 6))  //add by peng
        {
            if (event->u.dev.event == DEVICE_EVENT_IN) 
            {
                file_msg_hide(FILE_MSG_GAG);  //??? ??????????__sys_msg_hide????????????? BOX_MSG_NO_GAG?????????????????? 
            } 
            else if (event->u.dev.event == DEVICE_EVENT_OUT) 
            {
                file_msg_show(FILE_MSG_GAG);  //??? ??????????__sys_msg_hide????????????? BOX_MSG_NO_GAG??????????????????  
            }
        } 
    }
}
