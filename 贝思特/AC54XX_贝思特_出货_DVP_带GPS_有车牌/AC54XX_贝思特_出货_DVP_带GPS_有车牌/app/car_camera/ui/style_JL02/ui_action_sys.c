...
volatile int sys_in_lane_flag = 0;
...
static void sys_menu_show(enum SYS_MENU item)
{
    ...
    case SYS_MENU_VERSION:
        ...
        /* >>>>>>>>>>>>显示版本号 */
        #if (SLELE_5XX == JL5601)
        //ui_text_set_str_by_id(SYS_TXT_VERSION, "ascii", "VERSION: 5.9.6 20180922J");
       // ui_text_set_str_by_id(SYS_TXT_VERSION, "ascii", "56_WT_GT_08_20181029");
        //ui_text_set_str_by_id(SYS_TXT_VERSION, "ascii", "56_WT_GT_Z_08_20181212");
        //ui_text_set_str_by_id(SYS_TXT_VERSION, "ascii", "56_WT_GT_Z_08_20190103");
	    //ui_text_set_str_by_id(SYS_TXT_VERSION, "ascii", "56_WTIPS_GT_Z_08_20190103");
	    /*****cxy****begin*****56_WTIPS_GT_F_08_20190103 改为 56DVP-WTIPS-GT-F-07-20190103**/
		//ui_text_set_str_by_id(SYS_TXT_VERSION, "ascii", "56_WTIPS_GT_F_08_20190103");
		ui_text_set_str_by_id(SYS_TXT_VERSION,"ascii","56DVP-WTIPS-GT-F-07-20190103");
		/*****cxy****end***/
       //ui_text_set_str_by_id(SYS_TXT_VERSION, "ascii", "56_LX_GT_Z_08_20181212");
        #else
        ui_text_set_str_by_id(SYS_TXT_VERSION, "ascii", "56_WT_GT_08_20180630");
        #endif
        break;
    }
}
...
static void battery_event_handler(struct sys_event *event, void *priv)
{
    ...
    if (event->type == SYS_DEVICE_EVENT) {
        ...
        else if (!ASCII_StrCmp(event->arg, "disacc", 6))  //add by peng
        {
            if (event->u.dev.event == DEVICE_EVENT_IN) 
            {
                __sys_msg_hide(BOX_MSG_GAG);  //??? ??????????__sys_msg_hide????????????? BOX_MSG_NO_GAG?????????????????? 
            } 
            else if (event->u.dev.event == DEVICE_EVENT_OUT) 
            {
                __sys_msg_show(BOX_MSG_GAG, 10*1000);  //??? ??????????__sys_msg_hide????????????? BOX_MSG_NO_GAG??????????????????  
            }
        }
    }
}
...
static int sys_time_year_up_ontouch(void *ctr, struct element_touch_event *e)
{
    UI_ONTOUCH_DEBUG("**sys time ontouch**");
    switch (e->event) {
    case ELM_EVENT_TOUCH_DOWN:
        ...
        sys_menu_tim_change(SYS_MENU_TIME_YEAR, 1);
        break;
    case ELM_EVENT_TOUCH_MOVE:
        UI_ONTOUCH_DEBUG("ELM_EVENT_TOUCH_MOVE\n");
    ...
    case ELM_EVENT_TOUCH_UP:
        UI_ONTOUCH_DEBUG("ELM_EVENT_TOUCH_UP\n");
        sys_menu_tim_change(SYS_MENU_TIME_SET, 1);
        break;
    }
    return false;
}
REGISTER_UI_EVENT_HANDLER(SYS_YEAR_UP)
.ontouch = sys_time_year_up_ontouch,
};
static int sys_time_year_down_ontouch(void *ctr, struct element_touch_event *e)
{
...
REGISTER_UI_EVENT_HANDLER(SYS_YEAR_DOWN)
.ontouch = sys_time_year_down_ontouch,
};

static int sys_time_mon_up_ontouch(void *ctr, struct element_touch_event *e)
{
    ...
    case ELM_EVENT_TOUCH_DOWN:
        ...
        sys_menu_tim_change(SYS_MENU_TIME_MONTH, 1);
        break;
    case ELM_EVENT_TOUCH_MOVE:
        UI_ONTOUCH_DEBUG("ELM_EVENT_TOUCH_MOVE\n");
...
REGISTER_UI_EVENT_HANDLER(SYS_MON_UP)
.ontouch = sys_time_mon_up_ontouch,
};

static int sys_time_mon_down_ontouch(void *ctr, struct element_touch_event *e)
{
...
REGISTER_UI_EVENT_HANDLER(SYS_MON_DOWN)
.ontouch = sys_time_mon_down_ontouch,
};

static int sys_time_day_up_ontouch(void *ctr, struct element_touch_event *e)
{
    ...
    case ELM_EVENT_TOUCH_DOWN:
        ...
        sys_menu_tim_change(SYS_MENU_TIME_DAY, 1);
        break;
    case ELM_EVENT_TOUCH_MOVE:
        UI_ONTOUCH_DEBUG("ELM_EVENT_TOUCH_MOVE\n");
...
REGISTER_UI_EVENT_HANDLER(SYS_DAY_UP)
.ontouch = sys_time_day_up_ontouch,
};

static int sys_time_day_down_ontouch(void *ctr, struct element_touch_event *e)
{
...
REGISTER_UI_EVENT_HANDLER(SYS_DAY_DOWN)
.ontouch = sys_time_day_down_ontouch,
};


static int sys_time_hour_up_ontouch(void *ctr, struct element_touch_event *e)
{
    ...
    case ELM_EVENT_TOUCH_DOWN:
        ...
        sys_menu_tim_change(SYS_MENU_TIME_HOUR, 1);
        break;
    case ELM_EVENT_TOUCH_MOVE:
        UI_ONTOUCH_DEBUG("ELM_EVENT_TOUCH_MOVE\n");
...
REGISTER_UI_EVENT_HANDLER(SYS_HOUR_UP)
.ontouch = sys_time_hour_up_ontouch,
};

static int sys_time_hour_down_ontouch(void *ctr, struct element_touch_event *e)
{
...
REGISTER_UI_EVENT_HANDLER(SYS_HOUR_DOWN)
.ontouch = sys_time_hour_down_ontouch,
};

static int sys_time_min_up_ontouch(void *ctr, struct element_touch_event *e)
{
    ...
    case ELM_EVENT_TOUCH_DOWN:
        ...
        sys_menu_tim_change(SYS_MENU_TIME_MIN, 1);
        break;
    case ELM_EVENT_TOUCH_MOVE:
        UI_ONTOUCH_DEBUG("ELM_EVENT_TOUCH_MOVE\n");
...
REGISTER_UI_EVENT_HANDLER(SYS_SEC_UP)
.ontouch = sys_time_min_up_ontouch,
};

static int sys_time_min_down_ontouch(void *ctr, struct element_touch_event *e)
{
...
REGISTER_UI_EVENT_HANDLER(SYS_SEC_DOWN)
.ontouch = sys_time_min_down_ontouch,
};
/***************************** 显示系统时钟确认修改 ************************************/
...
{
    struct application *app;
	struct intent it;											//***add	by cxy***					
    int top = __this->vanish_line * 352 / LCD_DEV_HIGHT;		//***add	by cxy***
    int bottom = __this->car_head_line * 352 / LCD_DEV_HIGHT;	//***add	by cxy***
    ...
    case ELM_EVENT_TOUCH_UP:
        ...
        if(lane_entry_flag == 1)  
        {
			...
			menu_lane_det_set(top << 16 | bottom);			//***add	by cxy***保存设置
            printf("===============app_name=%s======================\n", app->name);
            if (app) {
                it.name = "video_system";
                it.action = ACTION_BACK;
	...
	//此部分打开会有引起设置车道偏移后进入录像卡死现象
#if 1
	if(sys_in_lane_flag != 1)
	{
		extern u8 home_rec_touch;
		  home_rec_touch = 1;
	        init_intent(&it);
