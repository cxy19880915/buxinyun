...
static int det_timer_cnt = 0,det_move = 0;
...
static void DETECT_MOVING_30S(void);



u8 acc_in_deley_rec = 0xff;   //acc in ????????
...
void video_home_post_msg(const char *msg, ...)
{
#ifdef CONFIG_TOUCH_UI_ENABLE
    union uireq req;
    va_list argptr;
...
static void DETECT_MOVING_30S(void)
{
	static int det_timer = 30;
	printf("\n  ------det_timer_cnt:%d\n",det_timer_cnt);
	if(move_start)
	{
		det_timer_cnt++;
		if(det_timer_cnt >= det_timer){
			det_timer_cnt = 0;
			sys_timeout_del(det_move);
			if (__this->state == VIDREC_STA_START) {
				video_rec_control(0);
			}
...
static void ve_server_event_handler(void *priv, int argc, int *argv)
{
    ...
    case VE_MSG_MOTION_DETECT_MOVING:
        ...
        if ((__this->state == VIDREC_STA_STOP) || (__this->state == VIDREC_STA_IDLE)) {
            move_start = 1;
            video_rec_control(0);
			//det_move = sys_timer_add(NULL,DETECT_MOVING_30S,1000);
        }
...
static u32 video_rec_get_fsize(u8 cycle_time, u16 vid_width)
{
    ...
    if(acc_in_deley_rec == 1)
        fsize  = ((video_rec_get_abr(vid_width)) * 3) * 8250;
    else
...
static int video_rec_del_old_file()
{
	...
	int cyc_flag = db_select("cyc");//add by ***cxy***
	...
	if(cyc_flag == 0){//add by ***cxy***  非循环录影时空间不够不能录影
    	return 1;
    }
...
static int video0_rec_start()
{
    ...
    /*
     *??,I??P???,?????(??MOV??????),
     *roio_xy :???????, [6:0]??x?? ,[14:8]??x??,[22:16]??y??,[30:24]??y??,?0??1?????
     * roio_ratio : ??????
     */
    ...
    */
    ...
    req.rec.roi.roio_ratio = 256 * 90 / 100 ; // 70
    req.rec.roi.roio_ratio1 = 256 * 95 / 100; // 90
    req.rec.roi.roio_ratio2 = 0;
    req.rec.roi.roio_ratio3 = 256 * 95 / 100;// 80
#endif
...
static int sd_timer1 = 0;

static void __sd_timer_func(void *p)
{
	u8 i;
	static u8 cnt  = 0;
...
static void __sd_timer1_func(void *p)
{
	u8 i;
	static u8 cnt  = 0;
	if (storage_device_ready() == 0) 
...
static int video_rec_start()
{
	...
	if(sd_timer1 == 0)
		sd_timer1 = sys_timer_add(NULL, __sd_timer1_func, 500);
       video_rec_post_msg("noMEM");
       //eturn VREC_ERR_START_FREE_SPACE;
	   goto __start_err3;
    }
...
static int video_rec_control(void *_run_cmd)
{
    ...
    if (storage_device_ready() == 0) {
        if (!dev_online(SDX_DEV)) {
            ...
            //deo_rec_post_msg("fsErr");
            if(sd_timer == 0)	
			sd_timer = sys_timer_add(NULL, __sd_timer_func, 500);
        }
        return 0;
    } else {
...
int lane_det_setting_disp()
{
    ...
    err = video0_disp_start(dis_w+128+128*2, SCREEN_H, 0/*((SCREEN_W - dis_w) / 2 / 16 * 16)*/, 0);
    show_lane_set_ui();
#endif
    return err;
}
...
static int video_rec_all_back_init()   //???????
{
    int err = 0;
...
static int video_disp_all_back()
{
    int err = 0;
...
extern volatile char if_in_rec;
/*
    ...
    case APP_STA_START:
        ...
        case ACTION_VIDEO_REC_MAIN:
	     ...
	     dec_main_flag = 0;
            ...
            break;
	case ACTION_VIDEO_REC_ALL_BACK:
            if (it->data && !strcmp(it->data, "lan_setting")) {
                __this->lan_det_setting = 1;
                ve_server_open(1);
                lane_det_setting_disp();
            } else {
                video_rec_all_back_init();
            }
            __this->run_rec_start = 1;
            break;
	case ACTION_VIDEO_REC_SWITCH:
            video_disp_all_back();
            break;
        case ACTION_VIDEO_REC_TO_HOME:
            __this->disp_state = DISP_BACK_WIN;
            video_disp_control();
            break;
         case ACTION_VIDEO_REC_TO_DOG:
            play_dog_voice_hander();
            break;    
        case ACTION_VIDEO_REC_SET_CONFIG:
            ...
            db_flush();
            break;
        case ACTION_VIDEO_REC_ACCOUT_CONTROL:   //add by peng  ?????????
           
            if(ui_get_current_window_id() == ID_WINDOW_MAIN_PAGE)
            {
                ui_hide(ID_WINDOW_MAIN_PAGE);
                ui_show(ID_WINDOW_VIDEO_REC);
            }
            
            if(acc_in_deley_rec == 0)
            {
                acc_in_deley_rec = 1;
                ui_lcd_light_off();
                if(if_in_rec)
            ...
            break;  
        case ACTION_VIDEO_REC_ACCIN_CONTROL:  //add by peng
            if(acc_in_deley_rec == 0xff)  //???????翪??
            {
                acc_in_deley_rec = 0;
                break;
            }
