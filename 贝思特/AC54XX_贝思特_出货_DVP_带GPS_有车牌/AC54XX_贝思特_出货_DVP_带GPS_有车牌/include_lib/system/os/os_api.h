...
#define  OS_TICK_STEP_WAIT            1u    /* Waiting for OS to set OSTickStepState to _ONCE  */
#define  OS_TICK_STEP_ONCE            2u    /* Process tick once and wait for next cmd from OS*/
#endif
...
typedef OS_EVENT OS_SEM, OS_MUTEX, OS_QUEUE;

extern void OSInitTick();
...
int os_task_pend(const char *format, ...);

#define os_taskq_pend(a, b, c) os_task_pend("timeout taskq", a, c, b)

u8  OSTaskQPost(char *name, int argc, ...);
u8  OSTaskQPostMsg(char *name, int argc, ...);
u8  OSTaskQPostEvent(char *name, int argc, ...);
u8 OSTaskQDel(const char *name, int type);
u8 OSTaskQPostType(const char *name, int type, int argc, ...);

int task_queue_post_event(char *name, void *data, int len);
int task_queue_post_msg(char *name, void *data, int len);
...
#define os_taskq_post_type(...)  OSTaskQPostType(__VA_ARGS__)

#define os_taskq_del_type(a,b)  OSTaskQDel(a, b)
...
const char *OSGetCurrentTask();
#define current_task() OSGetCurrentTask()
...
u32 get_msg(void);
u16 put_msg_fifo(u32 msg, u8 type, void *parm);
