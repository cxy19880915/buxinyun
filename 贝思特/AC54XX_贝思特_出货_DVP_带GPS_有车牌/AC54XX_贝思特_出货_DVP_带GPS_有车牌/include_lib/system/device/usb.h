...
struct usb_uvc_camera {
    ...
    u16 width[5];
    u16 height[5];
    int (*video_open)(int frame_id);
    int (*video_reqbuf)(void *buf, u32 len, u32 *frame_end);
...
#define USBIOC_UVC_CAMERA_CONNECT       _IOW('U', 1, sizeof(struct usb_uvc_camera))
#define USBIOC_SLAVE_MODE_START         _IOW('U', 2, sizeof(unsigned int))
#define USBIOC_SLAVE_DISCONNECT         _IOW('U', 3, sizeof(unsigned int))
#define UVCIOC_QUERYCAP                 _IOR('U', 4, sizeof(struct uvc_capability))
#define UVCIOC_SET_CAP_SIZE             _IOW('U', 5, sizeof(unsigned int))
#define UVCIOC_STREAM_ON                _IOW('U', 6, sizeof(unsigned int))
#define UVCIOC_STREAM_OFF               _IOW('U', 7, sizeof(unsigned int))
#define UVCIOC_REQBUFS                  _IOW('U', 8, sizeof(unsigned int))
#define UVCIOC_DQBUF                    _IOW('U', 9, sizeof(unsigned int))
#define UVCIOC_QBUF                     _IOW('U', 10, sizeof(unsigned int))
#define UVCIOC_RESET                    _IOW('U', 11, sizeof(unsigned int))
#define UVCIOC_REQ_PROCESSING_UNIT      _IOR('U', 12, sizeof(struct uvc_processing_unit))
#define UVCIOC_SET_PROCESSING_UNIT      _IOW('U', 13, sizeof(struct uvc_processing_unit))
#define UVCIOC_GET_DEVICE_ID            _IOR('U', 14, sizeof(struct usb_device_id))
