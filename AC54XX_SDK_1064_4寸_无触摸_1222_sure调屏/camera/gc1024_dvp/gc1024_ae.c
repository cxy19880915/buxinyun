...
extern u32 gc1024_calc_shutter(isp_ae_shutter_t *shutter, u32 exp_time_us, u32 gain);
extern u32 gc1024_set_shutter(isp_ae_shutter_t *shutter);
extern void  *gc1024_get_ae_curve(u32 type, u32 fps, int *min_ev, int *max_ev);


//Anti-flicker:50Hz

const static u32 gc1024_ae_50hz_25fps[][AE_CURVE_INFO_MAX] = {
...
const static u32 gc1024_ae_50hz_30fps[][AE_CURVE_INFO_MAX] = {
    // EV       ExpoTime    ISO     INTERPOLATE
    {9102000,     30,       100,    AE_INTERP_EXP},  // lv = 20
    {27306,    10000,       100,    AE_INTERP_GAIN}, // lv = 10.74
    {13653,    10000,       200,    AE_INTERP_GAIN},
...
const static u32 gc1024_ae_60hz_30fps[][AE_CURVE_INFO_MAX] = {
    // EV       ExpoTime    ISO     INTERPOLATE
    {9102000,     30,       100,    AE_INTERP_EXP},  //lv = 20
...
static isp_ae_params_t  gc1024_ae_params = {
    ...
    .ae_ops = {
        .calc_shutter = gc1024_calc_shutter,
        .set_shutter  = gc1024_set_shutter,
        .get_ae_curve = gc1024_get_ae_curve,
    },
};


void gc1024_ae_ev_init(u32 fps)
{
    u32 time, gain;
    int minev, maxev;
    ...
    ae_curve = gc1024_get_ae_curve(AE_CURVE_TYPE, fps, &minev, &maxev);
    isp_ae_curve_interp(ae_curve, AE_INIT_EV, &time, &gain);

    gc1024_calc_shutter(&ae_shutter, time, gain);

    gc1024_set_shutter(&ae_shutter);
}


void   *gc1024_get_ae_curve(u32 type, u32 fps, int *min_ev, int *max_ev)
{
    const void *curve;

    *min_ev = AE_MIN_EV;
    *max_ev = AE_MAX_EV;
    curve = gc1024_ae_50hz_25fps;
    ...
    if (type == AE_CURVE_50HZ) {
        ...
        *max_ev = AE_MAX_EV;
        ...
        if (fps == 25) {
            ...
            curve = gc1024_ae_50hz_30fps;
        }
    } else {
        *min_ev = AE_MIN_EV;
        *max_ev = AE_MAX_EV;

        curve = gc1024_ae_60hz_30fps;
...
void *GC1024_get_ae_params()
{
    return (void *)&gc1024_ae_params;
}
