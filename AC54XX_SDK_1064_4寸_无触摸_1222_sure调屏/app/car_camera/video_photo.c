...
#if 0
static int pic1_mic_onchange(void *ctr, enum element_change_event e, void *arg)
{
...
REGISTER_UI_EVENT_HANDLER(BASEFORM_87)
.onchange = pic1_mic_onchange,
 .onkey = NULL,
  .ontouch = NULL,
};
#endif
...
#else
        req.display.width 	= 800;//dc->rect.width;
        req.display.height 	= 480;//dc->rect.height;

     req.display.border_left  = 0;
        req.display.border_top   = 0;//(req.display.height - 480) / 2 / 16 * 16; // 0;
        req.display.border_right = 0;
        req.display.border_bottom =1;//(req.display.height - 480) / 2 / 16 * 16; // 0;
