...
//unsigned char scane_type[] = {SCENE_OUTCLUDY, SCENE_OUTCLUDY, SCENE_OUTCLUDY, SCENE_OUTCLUDY};

//添加isp效果对应段的阈值范围
#define USER_ISP_CFG_NIGHT_L    ((0 << 8))

#define USER_ISP_CFG_NIGHT_H    ((0 << 8) 	+ 90)//((0 << 8) 	- 0) //+ 100

#define USER_ISP_CFG_INDOOR_L   ((0 << 8) 	+ 110) //+ 130
#define USER_ISP_CFG_INDOOR_H   ((6 << 8) 	- 256) //(6 << 8)

#define USER_ISP_CFG_OUTCLUDY_L ((7 << 8) 	+ 256) //(6 << 8)
...
static void isp_scene_switch(void *arg, u8 force)
{
 ...
 extern int ispt_params_smooth_en(u8 en);
    ispt_params_smooth_en(0); // starting speed add on 20171222
...
int video_rec_set_wdr(struct server *server, u8 scene)
{
    ...
    if (wdr_en[scene])
	{
        req.camera.drc = 1;//db_select("wdr");//db_select("wdr");
    } else {
        req.camera.drc = 0;
    }
