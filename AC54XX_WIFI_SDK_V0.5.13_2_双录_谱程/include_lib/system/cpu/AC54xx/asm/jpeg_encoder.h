...
struct jpeg_encoder_fd {
    ...
    unsigned enable_abr : 1;
    unsigned enable_dyhuffman: 1;
    unsigned source_connected: 1;
    unsigned std_huffman: 1;
    unsigned frame_drop: 1;//判断当前帧是否是丢帧
    volatile unsigned mcu_pnd: 1;
...
int jpeg_encoder_reduce_bits_rate(void *_fd, int setp);
