...
static int timer_rec_remain_onchange(void *ctr, enum element_change_event e, void *arg)
{
    ...
    case ON_CHANGE_SHOW_PROBE:
        ...
        //录像停止时可能出现app task被抢占导致无法及时响应消息
        i = 0;
        do {
            init_intent(&it);
            it.name    = "video_rec";
        ...
        for (j = 0, i = 0; j < sizeof(timebuf) / sizeof(timebuf[0]); j++) {
            while (it.data[i] != '.' && it.data[i] != '\0') {
                if (it.data[i] >= '0' && it.data[i] <= '9') {
                    timebuf[j] *= 10;
                    timebuf[j] += it.data[i] - '0';
                }
                i++;
            }
            i++;
...
static int rec_flush_time_handler(const char *type, u32 arg)
{
