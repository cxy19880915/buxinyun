...
static int video_dec_del_current_file(struct video_dec_hdl *__this)
{
    ...
    fget_name(__this->req.dec.file, name, 20);
    sprintf(path, "%s%s", dec_path[__this->curr_dir][0], name);
    sprintf(buf, "status:1,path:%s%s", dec_path[__this->curr_dir][0], name);
    CTP_CMD_COMBINED(NULL, CTP_NO_ERR, "FILES_DELETE", "NOTIFY", buf);
