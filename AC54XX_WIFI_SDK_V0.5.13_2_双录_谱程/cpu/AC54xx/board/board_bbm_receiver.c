...
extern const struct device_operations a7130_dev_ops;
...
int sdmmc_0_io_detect(const struct sdmmc_platform_data *data)
{
    ...
    if (!init) {
        init = 1;
        gpio_direction_input(SD0_DET_IO);
        gpio_set_pull_up(SD0_DET_IO, 1);
        gpio_set_pull_down(SD0_DET_IO, 0);
    }

    return !gpio_read(SD0_DET_IO);
}
...
LCD_PLATFORM_DATA_BEGIN(lcd_data)
	.lcd_name = (u8*)"mipi_4lane_480_EK79030",
...
.interface = LCD_DVP_RGB,
	.lcd_io = {
        ...
        .backlight = IO_PORTB_13,
        /* .backlight = -1, */
        .backlight_value = 1,
		/* .lcd_reset = -1, */
		.lcd_reset = IO_PORTH_12,
        .lcd_cs    = -1,
        .lcd_rs    = -1,
        .lcd_spi_ck= -1,
        .lcd_spi_di= -1,
...
RF_PLATFORM_DATA_BEGIN(rf_data)
#ifdef CONFIG_P2418_ENABLE
	.name = (u8 *)"pan2418",
#else
	#ifdef CONFIG_A7196_ENABLE
	.name = (u8 *)"a7196",
	#else
	.name = (u8 *)"a7130",
	#endif
...
NET_PLATFORM_DATA_BEGIN(net_phy_data)
	.name = (u8 *)"rtl8201",
 	.speed = PHY_FULLDUPLEX_100M,
  	.mode  = RMII_MODE,
	.check_link_time = 300, //100ms
    .mac_addr = {0x40, 0x8d, 0x5c, 0x95, 0x04, 0xe3},
	.rmii_bus = {
		.phy_addr = 0x1,
		.clk_pin = IO_PORTE_02,
		.dat_pin = IO_PORTE_03,
...
RF_PLATFORM_DATA_BEGIN(rf_para_data)
	.name = (u8 *)"a7130",
	.width = 1280,
	.height = 720,
	.mem_size = 1*1024*1024,
...
WIFI_PLATFORM_DATA_BEGIN(wifi_data)
	.module = RTL8189E,
	.sdio_parm = SDIO_GRP_1 | SDIO_PORT_2 | SDIO_4_BIT_DATA /*|SDIO_DATA1_IRQ*/ |SDIO_CLOCK_20M,
...
REGISTER_DEVICES(device_table) = {
    ...
    { "rf_phy",&rf_phy_dev_ops, (void *) &rf_data},
...
void debug_uart_init()
{
    ...
    if (uart0_data.tx_pin == IO_PORTE_02) {
        IOMC1 &= ~(0x0f << 8);
#if 1
        gpio_direction_output(IO_PORTE_02, 0);
        gpio_set_pull_up(IO_PORTE_02, 1);
        gpio_set_pull_down(IO_PORTE_02, 1);
        gpio_set_die(IO_PORTE_02, 0);

#else

        gpio_direction_output(IO_PORTH_07, 0);
        gpio_set_pull_up(IO_PORTH_07, 1);
        gpio_set_pull_down(IO_PORTH_07, 1);
        gpio_set_die(IO_PORTH_07, 0);
#endif
    }
...
void board_init()
{
    ...
    gpio_direction_output(IO_PORTA_08, 1); // LCD stby pin output high
    /* gpio_direction_output(IO_PORTA_08, 0); // LCD stby pin output high */
