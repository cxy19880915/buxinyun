...
static void    uart_port_remap_func(void)
{
    ...
    /*
     **********************特殊串口输入重定向*********************
    ***以PB3为例，IOMC3的设置是选择由IOMC2的0-6决定映射到那个引脚*****
    ***IOMC2的值，4-6决定是第几组IO，A组为0，4-0位决定是该组的第几脚***
    */
#if 1
...
.baudrate = 115200,
 .port = PORTE_0_1,//涔熷彲閫夋嫨甯哥敤IO锛屼笉浣跨敤閲嶅畾鍚?
//.port_remap_func = uart_port_remap_func,
  .max_continue_recv_cnt = 1024,
   .idle_sys_clk_cnt = 100000,
    .clk_src = PLL_48M,
...
static const struct camera_platform_data camera0_data = {
    .xclk_gpio      = IO_PORTB_10,
    .reset_gpio     = IO_PORTB_08,
    .online_detect  = NULL,
    .pwdn_gpio      = IO_PORTB_09,
    .power_value    = 1,
...
.module = MARVEL_88W8801,
 .sdio_parm = SDIO_GRP_2 | SDIO_PORT_1 | SDIO_4_BIT_DATA /*|SDIO_DATA1_IRQ*/ | SDIO_CLOCK_16M,
  .wakeup_port = -1,
   .cs_port = -1,
    .power_port = IO_PORTG_02,
