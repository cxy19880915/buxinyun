...
LCD_PLATFORM_DATA_BEGIN(lcd_data)
	...
	.lcd_io = {
        ...
        .backlight = IO_PORTH_07,
        .backlight_value = 1,
		.lcd_reset = IO_PORTH_06,
        .lcd_cs    = -1,
        .lcd_rs    = -1,
        .lcd_spi_ck= -1,
        .lcd_spi_di= -1,
...
static const struct camera_platform_data camera0_data = {
    .xclk_gpio      = IO_PORTB_13,
	.reset_gpio     = IO_PORTE_02,
    .online_detect  = NULL,
	.pwdn_gpio      = IO_PORTE_03,
    .power_value    = 1,
...
#define START_KEY_IO 	IO_PORTG_07
...
WIFI_PLATFORM_DATA_BEGIN(wifi_data)
	.module = RTL8189E,
	.sdio_parm = SDIO_GRP_0 | SDIO_PORT_2 | SDIO_4_BIT_DATA /*|SDIO_DATA1_IRQ*/ |SDIO_CLOCK_40M,
	.wakeup_port = -1,
	.cs_port = -1,
	.power_port = -1,
...
void s_led_ctrl(unsigned char on_off)
{
    gpio_direction_output(IO_PORTH_12, on_off);
}

void wifi_led_ctrl(unsigned char on_off)
{
    gpio_direction_output(IO_PORTB_12, on_off);
}
...
//如果VM不支持flash的大小，实现下列函数
#if 0
//参数 flash型号
//返回 flash大小
u32 set_flash_size(u8 flash_id)
{
         while (1) {
            printf("no support flash for vm \n");
        }
    return -1;
}
#endif
...
REGISTER_DEVICES(device_table) = {
	...
	{ "iic1",  &iic_dev_ops, (void *)&hw_iic0_data},//gsensor
...
void debug_uart_init()
{
	...
	if (uart0_data.tx_pin == IO_PORTH_09) {
        IOMC1 &= ~(0x0f << 8);
        gpio_direction_output(IO_PORTH_09, 0);
        gpio_set_pull_up(IO_PORTH_09, 1);
        gpio_set_pull_down(IO_PORTH_09, 1);
        gpio_set_die(IO_PORTH_09, 0);
