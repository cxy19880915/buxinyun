...
WIFI_PLATFORM_DATA_BEGIN(wifi_data)
	.module = RTL8189E,
	.sdio_parm = SDIO_GRP_0 | SDIO_PORT_3 | SDIO_1_BIT_DATA /*|SDIO_DATA1_IRQ*/ |SDIO_CLOCK_16M,
	.wakeup_port =  -1,
	.cs_port =  -1,
	.power_port = -1,
...
void debug_uart_init()
{
    ...
    if (uart3_data.tx_pin == IO_PORTG_01) {
        IOMC1 &= ~(0x0f << 8);
        IOMC1 |= 0x3 << 8;
        gpio_direction_output(IO_PORTG_01, 0);
        gpio_set_pull_up(IO_PORTG_01, 1);
        gpio_set_pull_down(IO_PORTG_01, 1);
        gpio_set_die(IO_PORTG_01, 0);
...
void rtl_mp_test_set_uart(char **uart_name)
{
    if (uart_name) {
        *uart_name = "uart1";
    }
