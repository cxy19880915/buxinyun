...
static u8 wkup_reason(void *p_dev)
{
    ...
    if (ret0 != 0 || ret1 != 0) {
        printf("\n pr 1/2/3 wakeup pending pr1 %d  pr2/3 %d\n", ret0, ret1);
        if (ret0) {
...
int first_time_power_on()
{
    ...
    if (p_dev) {
        ...
        {
            if (!usb_is_charging()) {//第一次接电池上电不开�?仅接电池中途复位不开�?
...
static u8 usb_offline = 0;  //��¼�ǲ��ǰ�USB�ػ���
...
static void usb_charging_detect(void *arg)
{
    ...
    if (__this->state == CHARGING_STATE_ON) {
        ...
        ui_lcd_light_on_low(); //ն¼ъ        printf("CHARGING OUT\n");
    }
